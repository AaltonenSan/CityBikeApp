  ...
  const search = (stations: Station[]) => {
    if (name) {
      return stations.filter((station) =>
        station.nimi.toLowerCase().includes(name.toLowerCase())
      );
    } else if (address) {
      return stations.filter((station) =>
        station.osoite.toLowerCase().includes(address.toLowerCase())
      );
    } else {
      return stations;
    }
        ...
        <thead>
          <tr>
            <th style={{ width: '270px', height: '46px' }}>
              {searchByName
                ? SearchInput('name', inputRef)
                : TitleWithSearch('Name', handleNameSearch)}
            </th>
            <th style={{ width: '300px', height: '46px' }}>
              {searchByAddress
                ? SearchInput('address', inputRef)
